...
<p>Facets allow you to display multiple plots, which can be very useful to show differences between groups. Let's use the `facet_wrap` function and your histogram from Exercise Two to compare mass differences between Black-capped and Carolina chickadees.</p> 
...
<hr>
<p>Modify your density plot from Exercise Two. Use the function `facet_wrap` and the argument `nrow = 2` to assign Black-capped (BCCH) and Carolina chickadees (CACH) to separate, vertically aligned facets.</p>
<br>
...
<p>Even the best-looking plot can be marred by using unattractive titles. Let's modify the bar plot we've been working on, changing the x-axis title "spp" to "Species" using the `xlab` function:</p>
...
<hr>
<ol start = "a">
<li>Modify the density plot that you created in Exercise Three. Using the piping method, change the names "BCCH" and "CACH" to "Black-capped" and "Carolina" and plot the output:</li>
<br>
```{r exercise 4 factor levels}
birdMeasures %>%
  filter(spp %in% c('BCCH', 'CACH')) %>%
  mutate(spp = factor(
    spp,
  ...
  ggplot(aes(x = mass)) +
  geom_density(aes(fill = spp), alpha = 0.7) +
  facet_wrap(~spp, nrow = 2) +
...
```
<br>
<li>Modify the density plot that you created in Exercise Three. Add the title "Mass of Carolina and Black-capped chickadees" and capitalize the x- and y-axis titles. Assign the name `massDensity` to the plot.</li>
<br>
```{r exercise 4 titles}
...
<hr>
<p>Modify the `massDensity` plot that you created in Excercies Four. Use the `expand`, `limits`, and `breaks` arguments of the function `scale_y_continuous` to scale the y-axis such that the scale ranges from 0 to 0.7 and breaks occur at intervals of 0.1.</p>
<br>
```{r exercise 5 scale axis}
# Labels for massDensity:
...
<p>The default colors that ggplot uses are just plain ugly. Luckily it's easy to modify the colors to those of your choosing. Here we'll use the `values` attribute of the function `scale_fill_manual` on the histogram plot to specify the colors for the fill:</p>
...
<p>There really wasn't much improvement here. My preference is to hunt around for colors I like and grab their hex codes. For example, I used a "color-picker" app to extract the colors of Team Zissou's uniforms in movie The Life Aquatic.</p>
...
<img style="display: block;
        ...
        margin-right: auto;
	      ...
	      border-color: white;" src="rWorkshop/images/lifeAquatic.jpg" width="600" />
</div>

<p>My color-picker provided me with the values `#9EB8C5`, for the shirts, and `#F32017` for the hats. Using these values:</p>
...
```{r aquatic palette define, eval = FALSE}
zPalette <- c('#9EB8C5', '#F32017')
```
<br>

<p>You can then apply your color palette using:<p>

```{r aquatic palette use}
birdMeasures %>%
  filter(spp %in% c('BCCH', 'CACH')) %>%
  ggplot(aes(x = mass)) +
  geom_histogram(aes(fill = sex), bins = 20) +
...
<hr>
<p>Continue to modify the `massDensity` plot from Exercise Five. Use the `values` argument of the function `scale_fill_manual` to set the colors (<i>Note: You may use zPalette or any other colors of your choosing</i>).</p>
<br>
```{r exercise 6 colors}
# Labels for massDensity:
