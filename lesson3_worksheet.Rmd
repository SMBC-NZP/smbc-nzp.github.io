...
<p>For this lesson, we will use an example data frame of bird point count observations. This data frame includes a four letter abbreviation of species names, the foraging habits of the species, and the dietary class of the species.</p>
...
fileURL <- getURL('https://raw.githubusercontent.com/bsevansunc/workshop_languageOfR/master/birdHabits.csv')

# Read in the data:

birdHabits <- read.csv(text = fileURL)
...
<p>Writing <i>for loops</i> generally requires following these three steps:</p>
...
<br>
<ol>
<li>Use a <i>for loop</i> to run a logical test of each record to determine whether the observation is of a bark foraging bird.</li>
...
birdHabits[birdHabits$foraging == 'aerial',]$count
...
```{r, eval=FALSE}
foragingGuilds <- unique(birdHabits$foraging)
...
sum(birdHabits[birdHabits$foraging == foragingGuilds[4],]$count)
```
<br>

<p>Note that the above result is equivalent to our result when we subset the birdHabits dataset using row values.</p>
...
for(i in 1:length(foragingGuilds)){
 foragingCounts[i] <- sum(birdHabits[birdHabits$foraging == foragingGuilds[i],]$count)
}
```
<br>
