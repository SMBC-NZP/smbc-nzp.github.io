...
<p>Because the mass measurement varies by date, is an attribute of the bird as it relates only to a given the observation date. Therefore each measurement, and each bird measured on a given date, should be given its own row, as below.</p>
<br>
<div class = "row">
...
<p>The third normal rule states that, in addition to first and second normal rules being met, columns cannot be directly dependent on any other column. Thus columns are attributes of the primary key and only the primary key. The easiest example for this condition is date -- many data managers will include a year field in a data table, as below.</p>
<br>
<div class = "row">
...
<p>It's easy to look at -- it's very easy to visually compare treatment A with treatment B -- but is it tidy? Nope, not at all. We would have to do some awkward wrangling in base R to get this into a tidy format. Luckily, Hadley Wickham and his team created the `tidyverse` package in R with the aim of turning messy datasets like these into tidy ones.</p>
