...
```{r}
insectivoreCounts <-
  birdCounts[birdCounts$diet == 'insectivore',]
```
<li>How many species are insectivores?</li><br>
```{r}
length(
  unique(insectivoreCounts$species))
```
<li>Do steps one and two in this exercise in a single line of code.</li><br>
```{r}
length(
  unique(birdCounts[birdCounts$diet == 'insectivore',]$species))
```
...
```{r}
length(
  unique(insectivoreCounts$site))
```
<li>In what proportion of sites were insectivores observed?</li><br>
```{r}
length(
  unique(insectivoreCounts$site))/
...
### Exercise 2. Revisiting the World Health Organization's population dataset:
<br><br>
...
<li>Subset whoPopulation to the the year 2012.</li>
```{r}
whoPopulation[whoPopulation$year == 2012, ]
```
<br>
...
<li>Extract a vector of unique country names that contain the word "State".</li>
```{r}
unique(
  whoPopulation[str_detect(whoPopulation$country, 'State'), ]$country)
```
<br>

<li>Calculate the average (mean) population for countries that contain the word "State"</li>
```{r}
mean(
  whoPopulation[str_detect(whoPopulation$country, 'State'), ]$population)
```
<br>

<li>Generate a vector of countries that had a population of less than 20,000 in 2012.</li>
```{r}
unique(
  whoPopulation[whoPopulation$population < 20000 &
                  whoPopulation$year == 2012, ]$country)
```
<br>

<li>Use logic to determine which country had the highest population in 2012.</li>
```{r}

# This one was tricky in base R!

who2012 <-
  whoPopulation[whoPopulation$year == 2012, ]

who2012[who2012$population == max(who2012$population), ]$country
```
</ol>
<hr>
