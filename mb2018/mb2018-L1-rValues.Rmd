---
title: "Introductory lesson one: Values"
...
```{r combine, exercise = TRUE, exercise.lines = 5}
...
```{r mean_assignment, exercise = TRUE, exercise.lines = 5}
# Calculate the mean of numberObject (non-nested):
```

```{r mean_assignment-hint-1}
mean(...)
```

```{r mean_assignment-hint-2}
mean(numberObject)
```
</div>
...
```{r mean_nested, exercise = TRUE, exercise.lines = 5}
# Calculate the mean of c(1, 1, 2, 3) (nested):
```

```{r mean_nested-hint-1}
mean(...)
```

```{r mean_nested-hint-2}
mean(
  c(...)
)
```

```{r mean_nested-hint-3}
mean(
  c(1, 1, 2, 3)
)
```
...
```{r classType, exercise = TRUE, exercise.lines = 5}

# Describing an object containing numeric values, nested:
...
```{r orderedInt, exercise = TRUE, exercise.lines = 5}
...
```{r orderedDbl, exercise = TRUE, exercise.lines = 5}
...
```{r asInt_assignment, exercise = TRUE, exercise.lines = 5}
...
```{r asInt_nested, exercise = TRUE, exercise.lines = 5}
...
<br>
<p>Similarly, we can the function `as.numeric()` to convert a value to a double numeric value.</p>
...
```{r asNum_assignment, exercise = TRUE, exercise.lines = 5}
# Convert the integers 1:5 to numeric:

```

```{r asNum_assignment-hint-1}
as.numeric(...)
```

```{r asNum_assignment-hint-2}
as.numeric(1:5)
```

<p>Describe the object using a nested coding structure and the `class()` and `typeof()` functions.</p>

```{r asNum_describe, exercise = TRUE, exercise.lines = 5}
# Describe the above numeric object:

```

```{r asNum_describe-hint-1}
class(...)

typeof(...)
```

```{r asNum_describe-hint-2}
class(
  as.numeric(...)
)
...
```{r charMake, exercise = TRUE, exercise.lines = 5}
# Create an object representing the months, March, February, January, and Januray:
...
```{r charExplore, exercise = TRUE, exercise.lines = 5}
...
```{r charExplore-hint-1}
class(...)

typeof(...)
```

```{r charExplore-hint-2}
class(
  c(...)
  )
...
```{r makeFactor, exercise = TRUE, exercise.lines = 5}
...
```{r exploreFactor, exercise = TRUE, exercise.lines = 5}
# Explore the class and type of exampleFactor:

```

```{r exploreFactor-hint-1}
class(...)
...
```{r levelFactor_explore, exercise = TRUE, exercise.lines = 5}
# View the levels of exampleFactor:


```

```{r levelFactor_explore-hint-1}
levels(...)
```

```{r levelFactor_explore-hint-2}
...
```{r plotFactor_badLevels, exercise = TRUE, exercise.lines = 5}
# Plot exampleFactor:


```

```{r plotFactor_badLevels-hint-1}
plot(...)
```

```{r plotFactor_badLevels-hint-2}
plot(exampleFactor)
```
</div>
<br>
...
```{r factor_setLevels, exercise = TRUE, exercise.lines = 5}
# Set factor levels:

```

```{r factor_setLevels-hint-1}
factor(...)

```

```{r factor_setLevels-hint-2}
factor(
    c(...),
    ...
  )
...
```{r plotFactor_goodLevels, exercise = TRUE, exercise.lines = 5}
# Plot factor with levels coded:

```

```{r plotFactor_goodLevels-hint-1}
plot(...)
```

```{r plotFactor_goodLevels-hint-2}
plot(
  factor(...)
)
```

```{r plotFactor_goodLevels-hint-3}
plot(
  factor(
    c(...),
    ...
...
```{r plotFactor_goodLevels-hint-4}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = ...
...
```{r plotFactor_goodLevels-hint-5}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = c(...)
...
```{r plotFactor_goodLevels-hint-6}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = c('one', 'two', 'three')
...
```{r factor_setLevelsLabels, exercise = TRUE, exercise.lines = 5}
# Set factor levels and capitalize labels:

```

```{r factor_setLevelsLabels-hint-1}
factor(...)
```

```{r factor_setLevelsLabels-hint-2}
factor(
  ...,
  levels = ...,
  labels = ...
...
```{r factor_setLevelsLabels-hint-3}
factor(
  c('one', 'one', 'two', 'three'),
  levels = ...,
  labels = ...
...
```{r factor_setLevelsLabels-hint-4}
factor(
  c('one', 'one', 'two', 'three'),
  levels = c(...),
  labels = ...
...
```{r factor_setLevelsLabels-hint-5}
factor(
  c('one', 'one', 'two', 'three'),
  levels = c('one', 'two', 'three'),
  labels = c(...)
...
```{r factor_setLevelsLabels-hint-6}
factor(
  c('one', 'one', 'two', 'three'),
  levels = c('one', 'two', 'three'),
...
```{r plotFactor_goodLevelsLabels, exercise = TRUE, exercise.lines = 5}
# Set factor levels and capitalize labels:


```

```{r plotFactor_goodLevelsLabels-hint-1}
plot(...)
```

```{r plotFactor_goodLevelsLabels-hint-2}
plot(
  factor(...)
)
```

```{r plotFactor_goodLevelsLabels-hint-3}
plot(
  factor(
    ...,
    levels = ...,
...
```{r plotFactor_goodLevelsLabels-hint-4}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = ...,
...
```{r plotFactor_goodLevelsLabels-hint-5}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = c(...),
...
```{r plotFactor_goodLevelsLabels-hint-6}
plot(
  factor(
    c('one', 'one', 'two', 'three'),
    levels = c('one', 'two', 'three'),
...
```{r ft, exercise = TRUE, exercise.lines = 3}
FALSE

TRUE
```

```{r ftNumeric, exercise = TRUE, exercise.lines = 3}
as.numeric(FALSE)

as.numeric(TRUE)
```

```{r ftMean, exercise = TRUE, exercise.lines = 3}
mean(
  c(FALSE, TRUE, TRUE)
  )
```

```{r ftAdd, exercise = TRUE, exercise.lines = 3}
FALSE + TRUE
...
<div class = "reviewDiv">
<p><i class="fa fa-user-circle-o" aria-hidden="true" style = "font-size: 150%"></i> Use the logical operator `==` to test whether the each set of values is equivalent ($\equiv$). For example, the first set of expression would be written as `3 == 3`.</p>
